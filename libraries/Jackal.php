...
class JackalTimes {
	// The times that we're keeping track of (associative array)
	public static $marks = array();
	// The times that we're keeping track of (associative array)
	public static $dbMarks = array();

	/**
	 * Marks the end of the call with the repective id
	 *
	 * @param int $id Id of the mark to end
	 * @return NULL
	 */
	public static function endMark($id=0, $name=false) {
		$i = 0;

		if(!$id) {
			// Find the last mark without a finish
			for($i=count(self::$marks); $i>0; $i--) {
				$mark = self::$marks[$i-1];

				// See if this mark has a finish
				if(!@$mark["finish"]) {
					// Make sure the names match up
					if($name) if($name != $mark["name"]) continue;
					// This is our match, remember it
					$id = $i-1;
					break;
				}
			}
		...
		assert('$id');
		self::$marks[$id]["finish"] = microtime(true);
		self::$marks[$id]["test"] = $name;
	}
	...
	// |--------------------------------------------------------------------------
	/**
	* Erase the last mark entered
	*/
	public static function erase() {
		array_pop(self::$marks);
	}
	...
	// |--------------------------------------------------------------------------
	// |
	// | Write down the time that the event with $name happened
	// |
	// |--------------------------------------------------------------------------
	public static function mark($name) {
		self::$marks[] = array("name"=>$name, "time"=>microtime(true));
	}
	...
	// |--------------------------------------------------------------------------
	// |
	// | Return an array of the times marked
	// |
	// |--------------------------------------------------------------------------
	...
	/**
	 * Mark a database call
	 *
	 * @param string SQL of database call
	 * @return int The id of the mark
	 */
	public static function markDB($sql) {
		self::$dbMarks[] = array("sql"=>$sql, "start"=>microtime(true), "finish"=>0);
	 ...
	 * Get database calls
	 *
	 * @return array Associative array of database calls
	 */
	public static function getDBTimes() {
		return self::$dbMarks;
	}
}
